...
JL_DLLEXPORT jl_code_info_t *jl_code_for_staged(jl_method_instance_t *linfo)
{
    ...
    JL_TRY {
        ...
        ex = jl_call_staged(def, generator, linfo->sparam_vals, jl_svec_data(tt->parameters), jl_nparams(tt));
        ...
        jl_linenumber_to_lineinfo(func, (jl_value_t*)def);
    }
    JL_CATCH {
        ptls->in_pure_callback = last_in;
        jl_lineno = last_lineno;
...
static void jl_method_set_source(jl_method_t *m, jl_code_info_t *src)
{
    ...
    m->called = called;
    m->pure = src->pure;
    jl_linenumber_to_lineinfo(src, (jl_value_t*)m);
